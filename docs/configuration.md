...
All configuration for the MQTTtastic MQTT 5.0 client is done through `MqttConfig`, passed to `MqttClient` at construction time. Configuration is **immutable after creation** — use Kotlin's `copy()` on the data class if you need variants.
