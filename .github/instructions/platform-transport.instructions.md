...
- `MqttTransport` is an internal interface in `commonMain`. Concrete implementations live in `nonWebMain` (TcpTransport) and `wasmJsMain` (WebSocketTransport). There are no `expect`/`actual` declarations — transport selection is done via factory functions.
