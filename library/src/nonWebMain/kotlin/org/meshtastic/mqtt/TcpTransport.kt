    ...
    override suspend fun connect(endpoint: MqttEndpoint) {
        require(endpoint is MqttEndpoint.Tcp) {
            "TcpTransport only supports MqttEndpoint.Tcp. " +
                "For WebSocket endpoints, use MqttEndpoint.WebSocket — " +
        ...
        // Run the entire connect path on Dispatchers.IO. Ktor's TcpSocketBuilder.connect
        // resolves the hostname via a blocking java.net.InetSocketAddress constructor,
        // which throws NetworkOnMainThreadException under Android's StrictMode if called
        // on the Main dispatcher. Hopping to IO also keeps the TLS handshake off Main.
