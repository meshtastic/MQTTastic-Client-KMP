...
public class MqttClient
    public constructor(
        ...
        private var effectiveTransportFactory: (MqttEndpoint) -> MqttTransport = { endpoint ->
            (
                config.transportFactory ?: error(
                    "No MqttTransportFactory configured. Add a transport module " +
                        "(e.g. org.meshtastic:mqtt-client-transport-tcp) and set transportFactory " +
                        "in the client config, e.g. transportFactory = TcpTransportFactory().",
                )
            ).create(endpoint)
        }
